...
module "nat" {
  ...
  subnet_ids   = flatten([module.public_subnet.ids])
  subnet_count = length(var.public_subnet_cidrs)
}

resource "aws_route" "public_igw_route" {
  count                  = length(var.public_subnet_cidrs)
  route_table_id         = element(flatten([module.public_subnet.route_table_ids]), count.index)
...
resource "aws_route" "private_nat_route" {
  ...
  route_table_id         = element(flatten([module.private_subnet.route_table_ids]), count.index)
